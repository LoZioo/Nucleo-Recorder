...
//Nuove funzioni
void TIM_config_OC(TIM_TypeDef * tim, int chan, OC_Mode ocmode){
	switch (chan){
		case 1:
			tim->CCMR1 = (tim->CCMR1 & 0xff8f) | (ocmode<<4);
			break;

		case 2:
			tim->CCMR1 = (tim->CCMR1 & 0x8fff) | (ocmode<<12);
			break;

		case 3:
			tim->CCMR2 = (tim->CCMR2 & 0xff8f) | (ocmode<<4);
			break;

		case 4:
			tim->CCMR2 = (tim->CCMR2 & 0x8fff) | (ocmode<<12);
			break;
	}
}

void TIM_set_CCR_val(TIM_TypeDef * tim, int chan, int val){
	switch (chan){
		case 1:
			tim->CCR1 = val;
			break;
...
void TIM_set_OC_polarity(TIM_TypeDef * tim, int chan, OC_Polarity pol){
	switch(pol){
		case OC_ACTIVE_HIGH:
			tim->CCER |= (1<<((chan-1)*4+1));
			break;

		case OC_ACTIVE_LOW:
			tim->CCER &= ~(uint16_t)(1<<((chan-1)*4+1));
			break;
	}
}
//--------------
