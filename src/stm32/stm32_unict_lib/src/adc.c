...
//Nuove funzioni
void ADC_enable_irq(ADC_TypeDef * adc){
	NVIC_EnableIRQ(ADC_IRQn);	//Per abilitare l'interrupt del processore
	adc->CR1 |= (1<<5);	//Per abilitare l'interrupt della periferica
}

void ADC_disable_irq(ADC_TypeDef * adc){
	NVIC_DisableIRQ(ADC_IRQn);
	adc->CR1 &= ~(uint32_t)(1<<5);
}
//--------------
